...
*                         © Abap
* ======================================================================
* Name........: Atualização automática das taxas de câmbio
* Author......: Jonathan de Andrade
* Date........: 08.06.2016 10:45:22
*-----------------------------------------------------------------------
* Description: Atualização automática das taxas de câmbio
...
FORM f_get_http_data.
  ...
  IF sy-subrc <> 0.
    text1 = '-' ##NO_TEXT.
    text2 = 'Falha na comunicação.' ##NO_TEXT.
  ...
  IF sy-subrc = 0.
    text1 = '-' ##NO_TEXT.
    text2 = 'Arquivo não encontrado.' ##NO_TEXT.
    PERFORM f_registrar_log USING 'E' '047' 'ZFI' text1 text2 text3 text4.
    PERFORM f_save_log.
    PERFORM f_enviar_email.
    MESSAGE s047(zfi) DISPLAY LIKE 'E' WITH text1 text2.
...
FORM f_fill_data.
  ...
  IF r_cat_taxa[] IS INITIAL OR r_taxa[] IS INITIAL.
    MESSAGE 'Variáveis STVARV não cadastradas. Verifique.' TYPE 'S' DISPLAY LIKE 'E' ##NO_TEXT.
    LEAVE LIST-PROCESSING.
  ENDIF.
    ...
    CASE lw_cat_taxa-kurst.
      ...
      WHEN 'M'.
        ...
        LOOP AT lt_cotacao INTO lw_cotacao.
          ...
          lw_exch_rate-to_currncy = 'BRL'.
          lw_exch_rate-valid_from = lw_cotacao-camp01. " Data
          lw_exch_rate-exch_rate = ( lw_cotacao-camp05 + lw_cotacao-camp06 ) / 2. " Média Compra/Venda
          lw_exch_rate-from_factor = lw_tcurf-ffact.
          lw_exch_rate-to_factor = lw_tcurf-tfact.
          lw_exch_rate-exch_rate_v = ' '.
          lw_exch_rate-from_factor_v = lw_tcurf-ffact.
...
FORM f_chamar_bapi_create USING p_exch_rate.
  ...
  IF sy-subrc = 0.
    ...
    IF lw_bapiret2-type = 'E'.
      text1 = '-'.
      CONCATENATE 'Objeto bloqueado pelo usuário' lw_bapiret2-message_v1 INTO text2 SEPARATED BY space.
      PERFORM f_registrar_log USING 'E' '047' 'ZFI' text1 text2 text3 text4.
      PERFORM f_save_log.
      MESSAGE s047(zfi) DISPLAY LIKE 'E' WITH text1 text2.
      LEAVE LIST-PROCESSING.
...
FORM f_enviar_email.
  ...
  TRY.
      ...
      " Set body
      CONCATENATE 'Houve erro na importação da taxa de câmbio no SAP na data' lv_data '. Favor verificar e corrigir a transação OB08.'
                  'Para informações mais detalhadas consulte a transação SLG1: Objeto: Z_TAXA_CAMBIO, Subobjeto: ZTAXA.'
      ...
      lo_document = cl_document_bcs=>create_document(                            " Criação do documento
                                     ...
                                     i_subject = 'Erro na importação automática das taxas de câmbio' ). " Assunto do email
